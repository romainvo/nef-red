...
class ConvSpectralNorm(object):
    def __init__(self, name='weight', sigma=1.0, n_power_iterations=1,
                 dim=0, eps=1e-12, leakflag=False, kernelsize=3, padding=1):
        self.name = name
        self.sigma = sigma
        self.dim = dim
        if n_power_iterations <= 0:
    ...
    @staticmethod
    def apply(module, name, sigma, n_power_iterations, dim, eps, out_channels, leakflag, kernelsize, padding):
        fn = ConvSpectralNorm(name, sigma, n_power_iterations, dim, eps, leakflag, kernelsize, padding)
        weight = module._parameters[name]
        height = weight.size(dim)
...
def conv_spectral_norm(module, name='weight', sigma=1.0, n_power_iterations=1,
                       eps=1e-12, dim=None, out_channels = 64, leakflag=False, kernelsize=3, padding=1):
    r"""Applies spectral normalization to a parameter in the given module.
    .. math::
         \mathbf{W} &= \dfrac{\mathbf{W}}{\sigma(\mathbf{W})} \\
         \sigma(\mathbf{W}) &= \max_{\mathbf{h}: \mathbf{h} \ne 0} \dfrac{\|\mathbf{W} \mathbf{h}\|_2}{\|\mathbf{h}\|_2}
    ...
    if dim is None:
        if isinstance(module, (torch.nn.ConvTranspose1d,
            ...
            dim = 0
    ConvSpectralNorm.apply(module, name, sigma, n_power_iterations, dim, eps, out_channels, leakflag, kernelsize, padding)
    return module
